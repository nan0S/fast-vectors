...
template<class T>
class hybrid_allocator_lin : public allocator_base<hybrid_allocator_lin<T>, T> {
public:
    using base_t = allocator_base<hybrid_allocator_lin<T>, T>;
    using size_type = typename base_t::size_type;
    using value_type = T;
    using pointer = typename base_t::pointer;

    constexpr hybrid_allocator_lin() noexcept;
    constexpr explicit hybrid_allocator_lin(size_type n);
...
#ifdef UWR_TRACK
template<class T>
alloc_counter hybrid_allocator_lin<T>::counter("hybrid_allocator_lin");
#endif

template<class T>
constexpr
hybrid_allocator_lin<T>::hybrid_allocator_lin() noexcept
    : base_t() {}

template<class T>
constexpr
hybrid_allocator_lin<T>::hybrid_allocator_lin(size_type n)
    : base_t(n) {}

template<class T>
constexpr typename hybrid_allocator_lin<T>::size_type
hybrid_allocator_lin<T>::fix_capacity(size_type n) const {
#ifndef NDEBUG
    if (!n) return 0;
#endif
    if (this->is_big(n))
...
template<class T>
constexpr typename hybrid_allocator_lin<T>::size_type
hybrid_allocator_lin<T>::fix_big_capacity(size_type n) const {
...
template<class T>
constexpr typename hybrid_allocator_lin<T>::size_type
hybrid_allocator_lin<T>::fix_small_capacity(size_type n) const {
    UWR_ASSERT(!this->is_big(n));
...
template<class T>
constexpr int
hybrid_allocator_lin<T>::is_big(size_type n) const {
    return this->is_big(n, is_trivially_relocatable<T>());
}

template<class T>
constexpr int
hybrid_allocator_lin<T>::is_big(size_type n, true_type) const {
    return n * sizeof(T) >= page_size;
}

template<class T>
constexpr int
hybrid_allocator_lin<T>::is_big(size_type n, false_type) const {
    return n * sizeof(T) >= 32 * 1024 * 1024;
}

template<class T>
constexpr typename hybrid_allocator_lin<T>::pointer
hybrid_allocator_lin<T>::alloc(size_type n) const {
...
template<class T>
constexpr typename hybrid_allocator_lin<T>::pointer
hybrid_allocator_lin<T>::big_alloc(size_type n) const {
    UWR_ASSERT(this->is_big(n));
    UWR_ASSERT(n == this->fix_capacity(n));
...
template<class T>
constexpr typename hybrid_allocator_lin<T>::pointer
hybrid_allocator_lin<T>::small_alloc(size_type n) const {
    UWR_ASSERT(!this->is_big(n));
    UWR_ASSERT(n == this->fix_capacity(n));
...
template<class T>
constexpr void
hybrid_allocator_lin<T>::dealloc(pointer data, size_type n) const {
    UWR_ASSERT(n == this->fix_capacity(n));
...
template<class T>
constexpr void
hybrid_allocator_lin<T>::big_dealloc(pointer data, size_type n) const {
    UWR_ASSERT(this->is_big(n));
    UWR_ASSERT(n == this->fix_capacity(n));
...
template<class T>
constexpr void
hybrid_allocator_lin<T>::small_dealloc(pointer data, UWR_UNUSED size_type n) const {
    UWR_ASSERT(!this->is_big(n));
    UWR_ASSERT(n == this->fix_capacity(n));
...
template<class T>
constexpr void
hybrid_allocator_lin<T>::realloc(size_type req) {
...
template<class T>
constexpr typename hybrid_allocator_lin<T>::pointer
hybrid_allocator_lin<T>::do_realloc(size_type req, true_type) {
...
template<class T>
constexpr typename hybrid_allocator_lin<T>::pointer
hybrid_allocator_lin<T>::do_realloc(size_type req, false_type) {
    UWR_ASSERT(req > this->m_capacity);
    UWR_ASSERT(req == this->fix_capacity(req));
...
template<class T>
constexpr void
hybrid_allocator_lin<T>::grow(size_type req) {
    UWR_ASSERT(req > this->m_capacity);
...
template<class T>
constexpr void
hybrid_allocator_lin<T>::do_grow(size_type req, true_type) {
...
template<class T>
constexpr void
hybrid_allocator_lin<T>::do_grow(size_type req, false_type) {
    UWR_ASSERT(req > this->m_capacity);
    UWR_ASSERT(req == this->fix_capacity(req));
    ...
    switch (cond) {
        ...
        case 0b11: { /* both are big sizes */
            ...
            size_type step = req / 16;
            for (int i = 0; i < 8; ++i, cur_req -= step) {
                pointer new_data = (pointer)mremap(
                    this->m_data,
                    this->m_capacity * sizeof(T),
                    cur_req * sizeof(T), 0);
...
template<class T>
constexpr bool
hybrid_allocator_lin<T>::expand_or_dealloc_and_alloc_raw(size_type req) {
    UWR_ASSERT(req > this->m_capacity);
...
template<class T>
constexpr bool
hybrid_allocator_lin<T>::do_expand_or_dealloc_and_alloc_raw(size_type req, true_type) {
    UWR_ASSERT(req > this->m_capacity);
    UWR_ASSERT(req == this->fix_capacity(req));
...
template<class T>
constexpr bool
hybrid_allocator_lin<T>::do_expand_or_dealloc_and_alloc_raw(size_type req, false_type) {
    UWR_ASSERT(req > this->m_capacity);
    UWR_ASSERT(req == this->fix_capacity(req));
